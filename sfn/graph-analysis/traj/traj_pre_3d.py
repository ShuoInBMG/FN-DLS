...
class xyz_traj_handle:
    ...
    def detect_box_bounds(self, bound_slice):
        ...
        for line in bound_slice:
            ...
            if len(values) == 3:
                start, end, tilt = values
                tilt_factors.append(tilt)
            else:
                start, end = values
                tilt_factors.append(0.0)
            dimensions.append((start, end))
        return dimensions, tilt_factors
    def denormalize_coordinates(self, matrix, dimensions, tilt_factors):
        xy, xz, yz = tilt_factors
        for i in range(2, 5):
            start, end = dimensions[i-2]
            matrix[:, i] = matrix[:, i] * (end - start) + start
        matrix[:, 2] += matrix[:, 4] * xy + matrix[:, 3] * xz
        matrix[:, 3] += matrix[:, 4] * yz
        return matrix
    def extract_frames(self):
        ...
        for timestep in range(0, self.frames, self.timestep):
            ...
            dimensions, tilt_factors = self.detect_box_bounds(bound_slice)
            ...
            sorted_coords_de = self.denormalize_coordinates(sorted_coords, dimensions, tilt_factors)
